...
type Agent interface {
	...
	// Restart triggers a hot restart of daprd, applying the given config to the new process
...
func (a *agent) Restart(config interface{}) {
	...
	log.Infof("Received new config, creating new dapr runtime epoch %d", epoch)
	...
	// Unlock before the wait to avoid delaying daprd exit logic.
	a.mutex.Unlock()
...
func (a *agent) waitUntilLive(epoch int) {
	...
	// Timeout after 20 seconds. dapr runtime internally uses a 15s timer, so we set ours a bit above that.
		...
		case status := <-a.statusCh:
			...
			if status.err != nil {
				if status.err.Error() == errOutOfMemory {
					log.Warnf("dapr runtime may have been out of memory killed. Check memory usage and limits.")
				}
